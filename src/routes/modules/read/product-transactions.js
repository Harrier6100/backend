...
router.get('/', async (req, res, next) => {
    try {
        const field = req.query.field ? snakeize(req.query.field) : '*, sum(transaction_net_qty) over (partition by stock_id order by transaction_id) as transaction_net_qty';
        ...
        const productTransactions = await db.any(`
            select ${field} from product_transactions
            where ${filters.join(' and ')}
            order by ${sort}
        `, filterData);

        res.status(200).json(productTransactions);
    } catch (err) {
        next(err);
    }
});
