...
router.post('/', verifyToken, async (req, res, next) => {
    try {
        const exists = await db.oneOrNone(`
            select * from properties where id = \${id}
        `, { id: req.body.id });
        if (exists) throw new HttpError(req.t('MESSAGE.PROPERTY_EXISTS'), 409);
        ...
        await db.none(`
            insert into properties (${snakeize(Object.keys(property))})
            values (${Object.keys(property).map(c => '${' + c + '}')})
        `, property);
...
router.put('/:id', verifyToken, async (req, res, next) => {
    try {
        ...
        await db.none(`
            update properties set ${Object.keys(property).map(c => snakeize(c) + ' = ${' + c + '}')}
            where id = \${id}
        `, property);
